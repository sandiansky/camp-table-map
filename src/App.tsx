...
import { FixedObjectLibraryPage } from './pages/FixedObjectLibraryPage'
...
type Route = { page:'home'|'settings'|'library'|'objects' } | { page:'map'; zoneId:string; tableId?:string }
...
export default function App(){
 ...
 return <><div className="app-shell">{route.page==='home'&&<HomePage data={data} onOpenZone={(zoneId,tableId)=>navigate({page:'map',zoneId,tableId})} onCreateZone={()=>setZoneForm(null)} onEditZone={setZoneForm} onSettings={()=>navigate({page:'settings'})} onLibrary={()=>navigate({page:'library'})} onObjects={()=>navigate({page:'objects'})} onLoadSample={()=>setData(sampleData())}/>} {route.page==='library'&&<TableLibraryPage data={data} onBack={()=>navigate({page:'home'})} onPick={table=>navigate({page:'map',zoneId:table.zoneId,tableId:table.id})} update={update}/>} {route.page==='objects'&&<FixedObjectLibraryPage data={data} onBack={()=>navigate({page:'home'})} update={update}/>} {route.page==='settings'&&<SettingsPage data={data} onBack={()=>navigate({page:'home'})} onName={name=>update(d=>({...d,settings:{...d.settings,campName:name}}))} onImport={value=>{setData({...emptyData(),...value,templates:value.templates??[],landmarks:value.landmarks??[]});navigate({page:'home'})}} onClear={()=>setClearConfirm(true)}/>} {route.page==='map'&&zone&&<MapPage zone={zone} data={data} initialTableId={route.tableId} onBack={()=>navigate({page:'home'})} update={update}/>} {route.page==='map'&&!zone&&<div className="missing"><p>片区不存在或已被删除。</p><button className="btn primary" onClick={()=>navigate({page:'home'})}>返回首页</button></div>}</div>
