import { LibraryBig, MapPin, MoreHorizontal, Plus, Shapes, Settings } from 'lucide-react'
import { useState } from 'react'
import type { CampData, TableItem, Zone } from '../types'
import { SearchBox } from '../components/SearchBox'

export function HomePage({ data, onOpenZone, onCreateZone, onEditZone, onSettings, onLibrary, onObjects, onLoadSample }: {
  data: CampData; onOpenZone: (zoneId: string, tableId?: string) => void; onCreateZone: () => void;
  onEditZone: (zone: Zone) => void; onSettings: () => void; onLibrary: () => void; onObjects: () => void; onLoadSample: () => void
}) {
  ...
  return <div className="page home-page">
    <header className="home-head"><div><small>{data.settings.campName}</small><h1>桌位地图</h1><p>快速找到每一张桌子</p></div><div className="home-actions"><button className="icon-btn" onClick={onLibrary} aria-label="桌位库"><LibraryBig size={21} /></button><button className="icon-btn" onClick={onObjects} aria-label="固定物库"><Shapes size={21} /></button><button className="icon-btn" onClick={onSettings} aria-label="设置"><Settings size={22} /></button></div></header>
